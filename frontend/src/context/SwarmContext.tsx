        ...
        case "SSE_EVENT": {
            ...
            switch (evt.type) {
                case "status":
                    return {
                        ...state,
                        agentStatuses: { ...state.agentStatuses, [evt.agent_id]: evt.status },
                    };
                ...
                case "thought":
                    return {
                        ...state,
                        thoughts: [
                            { agent_id: evt.agent_id, text: evt.text, timestamp: evt.timestamp || new Date().toISOString() },
                            ...state.thoughts,
                        ].slice(0, 50),
                    };
                ...
                case "report_section":
                    switch (evt.section) {
                        ...
                        case "executive_summary":
                            return { ...state, executiveSummary: (evt.data as { summary: string }).summary };
                        default:
                            return state;
                    }
                ...
                case "chat_response":
                    if (evt.done) {
                        ...
                        return { ...state, chatMessages: [...state.chatMessages, msg], chatStreaming: "" };
                    }
...
export function SwarmProvider({ children }: { children: ReactNode }) {
    ...
    const triggerReport = useCallback(() => {
        dispatch({ type: "RESET" });
        // Re-trigger SSE — in real app this would POST to /trigger
        const backendUrl = import.meta.env.VITE_API_URL as string | undefined;
        if (backendUrl) {
            fetch(`${backendUrl}/trigger`, { method: "POST" }).catch(console.error);
        }
        // With mock, just re-connect
        setTimeout(() => {
            const cleanup = connectSSE((event) => {
                dispatch({ type: "SSE_EVENT", event });
            });
            // Store cleanup for later — simplified for hackathon
            return cleanup;
        }, 100);
    }, []);
